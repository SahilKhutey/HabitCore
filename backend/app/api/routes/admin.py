...
def get_retention_cohorts(db: Session = Depends(get_db)):
    ...
    SELECT 
        ...
        COUNT(DISTINCT CASE WHEN activity_date = signup_date + INTERVAL '1 day' THEN user_id END) AS d1,
        COUNT(DISTINCT CASE WHEN activity_date = signup_date + INTERVAL '7 day' THEN user_id END) AS d7
    FROM activity
